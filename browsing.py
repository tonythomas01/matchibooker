...
class Browser:
    ...
    def matchi_login(self, username, password):
        ...
        if (
            'id="loginForm"' not in self.browser.get_current_page().prettify()
    ...
    def get_available_slot_urls(self, day):
        ...
        timeslot_elements = self.browser.get_current_page().findAll(
            "a", {"class": available_time_class}
        )
        timeslot_urls = []
    ...
    def book_slot(self, slot):
        ...
        try:
            player_email = booking_page.find("input", {"name": "playerEmail"}).get(
                "value"
            )
            coupon_id = booking_page.find("option").get("value")
            order_id = booking_page.find("input", {"name": "orderId"}).get("value")
            facility_id = booking_page.find("input", {"name": "facilityId"}).get(
                "value"
            )
            ...
            booking_payload = {
                ...
                "end": end,
                "method": "COUPON",
                "customerCouponId": coupon_id,
                "playerEmail": player_email,
            }
            booking_response = self.browser.post(
                default_payment_url,
